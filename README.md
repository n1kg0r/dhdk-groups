...
def task_create(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect(reverse("projects:task_list"))
    else:
        form = TaskForm()
...
def task_list(request):
    ...
    return render(request, "projects/task_list.html", { "projects": tasks,})


# Retrieve a single task
def task_detail(request, pk):
    task = get_object_or_404(Task, pk=pk)
    return render(request, "projects/task_detail.html", { "project": task, })
...
def task_update(request, pk):
    ...
    return render(request, "projects/task_form.html", { "form": form, "object": task_obj})
